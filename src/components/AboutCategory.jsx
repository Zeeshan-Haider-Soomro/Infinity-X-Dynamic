    ...
    <section className="text-white py-8 md:py-16 px-4 md:px-16">
      <div className="text-center md:text-start max-w-7xl mx-auto grid md:grid-cols-2 gap-12">
        ...
        <div className="md:translate-y-65 translate-y-10">
          {/* Render the sectionHeading directly without wrapping h2 */}
          {sectionHeading}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {cards.map((card, index) => (
            <div
              key={index}
              className={`
                bg-[linear-gradient(90deg,rgba(26,1,39,1)_0%,rgba(60,9,69,1)_100%)] 
                px-4 py-8 rounded-lg shadow-md 
                transition-all duration-500 hover:bg-white 
                group cursor-pointer text-center 
                hover:text-[#dcb3e7] hover:shadow-lg 
